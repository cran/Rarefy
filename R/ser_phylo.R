...
ser_phylo<-function(comm,tree=NULL,dist_xy,method=c("faith", "barker", "Ia", "hill", "tsallis", "renyi","fun_div"),exp=0,fun_div=NULL,args=NULL,verbose=FALSE,comparison=FALSE) {
  ...
  if(comparison) {
    rare1<-rare_phylo(comm,tree,method=method,exp=exp,fun_div=fun_div,args=args)
    IC_up<-rare + (1.96*(sd(r_fin)/sqrt(nrow(comm))))
    IC_low<-rare - (1.96*(sd(r_fin)/sqrt(nrow(comm))))
    df<-data.frame(rare,IC_up,IC_low,rare1)
    colnames(df)<-c('Rarefaction_spaz','IC_up_spaz','IC_low_spaz','Rarefaction','IC_up','IC_low')
  }
